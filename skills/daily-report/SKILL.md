...
1. **メモリーを確認**: `memory/` 配下に保存された過去の調査範囲設定（対象リポジトリ、Jira プロジェクト、Slack チャンネル等）があれば読み込む。
2. **調査範囲の設定**:
   - **メモリーに前回の設定がある場合**: ステップを止めずに「〇〇の範囲で調査します。変更があればお知らせください」と宣言し、そのまま Step 3 に進む。ユーザーが変更を申し出た場合のみ反映する。
   - **メモリーに設定がない場合**: 各データソースについて調査対象を尋ねる（例: GitHub の対象リポジトリ、Jira の対象プロジェクト、Slack の対象チャンネルなど）。特に指定がなければデフォルト動作（全体検索またはスキップ）で進める。
3. **メモリーに保存**: ユーザーの回答が前回と異なる場合、または初回の場合は、調査範囲をメモリーに保存する（`memory/` 配下に保存し `MEMORY.md` のインデックスを更新する）。
