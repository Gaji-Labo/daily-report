...
**GitHub** — `gh` CLI を使い、対象日のアクティビティを取得する。

**a) PR・Issue の検索** (並列実行可):
```bash
...
**c) 自分が書いた PR コメント** (コメント先の PR を検索し、コメント内容を取得):
```bash
# 自分がコメントした PR を検索
...
# 各 PR のコメントから自分の当日分を抽出
gh api '/repos/{owner}/{repo}/issues/{number}/comments' --jq '[.[] | select(.user.login == "{username}" and (.created_at | startswith("YYYY-MM-DD")))] | .[] | {body: .body, created_at: .created_at}'
```

**d) 自分の PR に対する他者からのレビュー** (a で取得した自分の PR に対して実行):
```bash
# 各 PR のレビュー（承認・変更要求・コメント）を取得
...
環境変数 `TARGET_REPOS` が設定されていればカンマ区切りで `--repo` フラグを付与する (a, b の検索に適用)。
c, d は検索結果のリポジトリ情報から自動的にスコープが決まるため `TARGET_REPOS` 不要。
